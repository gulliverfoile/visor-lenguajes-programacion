...
async function handleRemoveLogs() {
    // Preguntar al usuario qué acción desea
    const accion = confirm('¿Eliminar las líneas de console.log? (Aceptar = eliminar, Cancelar = comentar)');
    const editor = getEditor();
    ...
    let newCode = code;
    // Ordenar de atrás hacia adelante para no alterar índices
    logProblems.sort((a, b) => b.line - a.line).forEach(problem => {
        // Si el usuario eligió eliminar, usamos el fixer especial; si no, el de comentar
        const ruleToUse = accion ? 'console-log-remove' : 'console-log';
        const fixed = applyFix({ ...problem, rule: ruleToUse }, newCode);
